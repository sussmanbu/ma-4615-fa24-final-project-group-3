...
model_race <- lm(
  Mean_earnings_After_10_Years ~ Perc_Undergrad_White + Perc_Undergrad_Black + Perc_Undergrad_Hispanic + Perc_Undergrad_Asian,
  data = education_cohort_data_cleaned
)
summary(model_race)

education_cohort_data_cleaned <- education_cohort_data_cleaned %>%
  mutate(Mean_earnings_After_10_Years = as.numeric(as.character(Mean_earnings_After_10_Years)))
...
### Linear Model of Racial disparities in post-graduation earnings
```{r}
str(education_cohort_data_cleaned$Med_Loan_Debt)
```
