...
The dataset aggregates data for different institutions across U.S. states.

To calculate the earnings for each race, I used weighted averages:

Weighted Earnings for Race} = {Population} * {Proportion of Race} * {Average Earnings}

The next step was to summarize the data at the state level:

	1.	Sum the Weighted Earnings for each race.
	2.	Sum the Weighted Population for each race.
	3.	Compute the Average Earnings for each race in a state:

{Average Earnings} = {Sum of Weighted Earnings} / {Sum of Weighted Population}

## Key insights of the map
	1.	Racial Disparities Exist:
	•	White and Asian graduates generally earn more on average compared to Black and Hispanic graduates.
	•	This suggests systemic disparities in earnings that may reflect broader socioeconomic and racial inequities.
	2.	Geographic Patterns:
	•	Earnings vary significantly across U.S. regions:
	•	States in the Northeast and West Coast generally have higher earnings.
	•	The South and parts of the Midwest consistently show lower earnings across all races.
