...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      plotOutput("earningsMap", height = "700px", 
...
server <- function(input, output, session) {
  ...
  # Show details upon clicking a state
  output$click_text <- renderText({
    click <- input$plot_click
    if (is.null(click)) {
      return("Click on a state to see its average earnings details.")
    }
    
    # Find the closest region to the mouse pointer based on a small bounding box
    clicked_state <- earnings_map_data %>%
      filter(
        long >= click$x - 0.5 & long <= click$x + 0.5,
        lat >= click$y - 0.5 & lat <= click$y + 0.5
      ) %>%
      distinct(region, Avg_Earn_White, Avg_Earn_Black, Avg_Earn_Hispanic, Avg_Earn_Asian) %>%
      head(1)
    
    if (nrow(clicked_state) == 0) return("No state found at this location.")
    
    state_info <- clicked_state[1, ]
