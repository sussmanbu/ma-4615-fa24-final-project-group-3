...
Analyzing higher education data provides valuable insight into a population of students who have opted to pursue higher education, identifying patterns that inform if there exists a fundamental difference in income and social status based upon student characteristics, such as race and pre-college income bracket. This data can further inform if there perhaps lie varying degrees of difficulty to succeed financially for certain groups over other groups, leading back to the question: Does the American Dream in higher education exist?
...
## So What's the Big Deal: Does Racial Disparity Even Exist?
```{r}
...
Post-Graduation earnings across the U.S. colleges are correlated to student demographic factors, specifically socioeconomic status and race. In particular, students from lower income brackets have lower post-graduation earnings than students from high income brackets *(I)*. Further, Black and Hispanic students have lower post-graduation earnings than their White and Asian counterparts *(II)*.

### I. Socioeconomically Diverse Institutions Have Lower Mean Earnings
...
```
*Percent Pell Grant and Socioeconomic diversity:* Defined in the data set as the share of degree/certificate-seeking undergraduate students who received Pell Grants in a given year. **Pell Grants** are awarded only to undergraduate students who display significent financial need and, unlike a loan, does not have to be repaid. This serves as an important measure of the access an institution provides to low-income students, thereby an indicator of an institution's socioeconomic diversity. Read more about Pell Grants [here](https://studentaid.gov/understand-aid/types/grants/pell).

The bar graph conveys the following idea: Institutions who support more lower-income students, albeit more socioeconomically diverse student bodies, have lower post-graduation earnings. This provides evidence that student affluence level prior to college is related to post-grad earnings.
...
ggplot(education_data, aes(x = Highest_Perc_Race, y = Avg_Mean_Earnings, color = Highest_Perc_Race)) +
  ...
  labs(
    title = "Average Student Mean Earnings Across U.S. Institutions",
...
```
*Average Earnings After College:*  Defined in the dataset as the mean earnings for students at each institution, each year starting 6 years after a student enrolls in college, up to 10 years after the student enrolls. Enrollment dates are estimated based on FAFSA self-reporting. This data analysis takes the average of all mean earnings from 6 to 10 years to provide a centralized value for student earnings from 6-10 years after enrollment at each institution.

The plot supports the following idea: There is a difference in post-grad earnings for schools based on their dominant race student population. Specifically, institutions with dominant Asian or White populations tend to have higher median earnings compared to those with dominant Black or Hispanic populations.


## Post-College Earnings Across the United States
```{shinylive-r}
#| standalone: true
#| viewerHeight: 640
#| echo: false
...
# UI definition
ui <- fluidPage(
  titlePanel("Mean Earnings by Race Across States"),
  sidebarLayout(
    sidebarPanel(
      helpText("Select a racial group to view their mean earnings across the United States"),
      selectInput(
        "race_variable",
...
The dataset `education_cohort_data_cleaned.rds` contains:
	...
	•	Average Earnings (Avg_Mean_earning): The mean income for students 6-10 years after enrollment at the institution

Earnings for each race were calculated using weighted averages:

Weighted Earnings for Race} = {Population} * {Proportion of Race} * {Average Earnings}

Data were summarized at the state level:

	1. Sum the Weighted Earnings for each race.
	2. Sum the Weighted Population for each race.
	3. Compute the Average Earnings for each race in a state: {Average Earnings} = {Sum of Weighted Earnings} / {Sum of Weighted Population}

#### Key Insights
1. Racial Disparity in Mean Earnings:
- White and Asian students earn higher post-college wages, on average, compared to their Black and Hispanic counterparts
- This suggests that there exists racial disparity in post-college earnings
2. Geographic Patterns:
- Earnings vary significantly across U.S. regions
- States in the Northeast and West Coast generally have higher earnings, while the South and parts of the Midwest consistently show lower earnings across all races
