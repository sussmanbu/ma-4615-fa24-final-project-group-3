...
# Create earnings columns for each race
education_data <- education_data %>%
  mutate(
    Earn_White = as.numeric(Perc_Undergrad_White) * as.numeric(Avg_Mean_earning),
    Earn_Black = as.numeric(Perc_Undergrad_Black) * as.numeric(Avg_Mean_earning),
    Earn_Hispanic = as.numeric(Perc_Undergrad_Hispanic) * as.numeric(Avg_Mean_earning),
    Earn_Asian = as.numeric(Perc_Undergrad_Asian) * as.numeric(Avg_Mean_earning),
    Earn_Two_Or_More_Races = as.numeric(Perc_Two_Or_More_Races) * as.numeric(Avg_Mean_earning)
  )

# Summarize data by state to calculate average earnings
state_earnings_summary <- education_data %>%
  group_by(State) %>%
  summarise(
    Num_Schools = n(),  # Number of schools in each state
    Total_Earn_White = sum(Earn_White, na.rm = TRUE),
    Total_Earn_Black = sum(Earn_Black, na.rm = TRUE),
    Total_Earn_Hispanic = sum(Earn_Hispanic, na.rm = TRUE),
    Total_Earn_Asian = sum(Earn_Asian, na.rm = TRUE),
    Total_Earn_Two_Or_More_Races = sum(Earn_Two_Or_More_Races, na.rm = TRUE)
  ) %>%
  mutate(
    Avg_Earn_White = Total_Earn_White / Num_Schools,
    Avg_Earn_Black = Total_Earn_Black / Num_Schools,
    Avg_Earn_Hispanic = Total_Earn_Hispanic / Num_Schools,
    Avg_Earn_Asian = Total_Earn_Asian / Num_Schools,
    Avg_Earn_Two_Or_More_Races = Total_Earn_Two_Or_More_Races / Num_Schools,
